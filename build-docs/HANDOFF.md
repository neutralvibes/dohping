...
| 79 | Plain-mode state-change clobber: `resizeMarkAbove` fired on EVERY finalize (not just resizes), replacing the header/finalized history with `-`; now gated on a real resize. **ACCEPTED 2026-08-23 after ~40 state changes + restarts + resize** | "prints a blank line with '-', losing the header. It also only shows 2 lines at a time" |
